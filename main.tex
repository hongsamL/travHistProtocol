...
Different combinations of transition kernels can be used to fix certain parameters %PL: this a bit oddly formulated. I would explicitly say that to 'fix' parameters we remove transition kernels on these parameters, in whihc case their starting values serve as the fixed values
...
\ann{Here, we run BEAST on the command line using the latest build of BEAST. You can create your own beast.jar file by checking out and compiling the main branch of the \code{beast-mcmc} GitHub repository. We specify a starting seed with the -seed flag, and use the \code{-beagle\_cuda} flag %PL: you mean the -beagle_gpu flag, right?
...
\hspace{0.5cm}R with package MarkovJumpR %PL: add URL?

\subsubsection*{Files}
Trees output from a BEAST phylogeographic analysis with travel history\\
...
This section is optional but I'm open for suggestions. %PL: either drop it or simply invite the reader to go an have a look at the tutorials on beast.community to explore other analyses.
